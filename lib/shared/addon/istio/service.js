...
export default Service.extend({
  ...
  checkKialiUiEndpoint(clusterId, force = false) {
    ...
    const preIsitio142BreakingKialiUrl = globalStore.rawRequest({
      url:    `/k8s/clusters/${ clusterId }/api/v1/namespaces/istio-system/services/http:kiali-http:80/proxy/kiali`,
      method: 'GET',
    });
    const postIsitio142BreakingKialiUrl = globalStore.rawRequest({
      url:    `/k8s/clusters/${ clusterId }/api/v1/namespaces/istio-system/services/http:kiali:20001/proxy/kiali`,
      method: 'GET',
    });
